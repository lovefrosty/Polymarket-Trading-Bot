...
from dashboard.panels.analyst_scan import render_analyst_scan_view
from dashboard.panels.market_context import render_market_context_panel
from dashboard.panels.ibkr_portfolio import render_ibkr_execution_view
...
from dashboard.panels.trading_workstation import (
    render_idea_inbox_view,
    render_macro_view,
    render_portfolio_view as render_broker_portfolio_view,
    render_rebalance_view,
    render_risk_view,
)
from dashboard.theme import PORTFOLIO_TERMINAL_CSS
...
def render_dashboard() -> None:
    ...
    st.set_page_config(
        page_title="HERMES TRADING TERMINAL",
        layout="wide",
        page_icon="",
        initial_sidebar_state="collapsed",
    )
    st.markdown(TERMINAL_CSS, unsafe_allow_html=True)
    st.markdown(PORTFOLIO_TERMINAL_CSS, unsafe_allow_html=True)
    st.markdown(
        '<h1 style="margin:0 0 2px;">HERMES // PORTFOLIO COMMAND TERMINAL</h1>',
        unsafe_allow_html=True,
    )
    st.caption("LIVE PORTFOLIO STATE // RISK // MACRO // AGENT IDEAS // HUMAN-REVIEWED EXECUTION")
    ...
    def _render_live() -> None:
        ...
        db = _current_db_path()
        portfolio_tab, risk_tab, macro_tab, analyst_tab, ideas_tab, rebalance_tab, execution_tab = st.tabs(
            ["PORTFOLIO", "RISK", "MACRO", "ANALYST SCAN", "IDEA INBOX", "REBALANCE", "EXECUTION"]
        )

        with portfolio_tab:
            _render_panel("portfolio_workstation", render_broker_portfolio_view, budget_ms=1000)
        with risk_tab:
            _render_panel("portfolio_risk", render_risk_view, budget_ms=1000)
        with macro_tab:
            _render_panel("macro_regime", render_macro_view, budget_ms=600)
        with analyst_tab:
            _render_panel("analyst_scan", render_analyst_scan_view, budget_ms=700)
        with ideas_tab:
            _render_panel("idea_inbox", render_idea_inbox_view, budget_ms=600)
        with rebalance_tab:
            _render_panel("rebalance", render_rebalance_view, budget_ms=800)
        with execution_tab:
            ibkr_exec, prediction_exec, mm_exec, alpha_exec = st.tabs(
                ["IBKR PAPER", "PREDICTION PORTFOLIO", "MARKET MAKING", "ALPHA OVERLAY"]
            )
            with ibkr_exec:
                _render_panel("ibkr_execution", render_ibkr_execution_view, budget_ms=900)
            with prediction_exec:
                _render_panel(
            ...
            with alpha_exec:
                _render_panel("alpha_overlay", lambda: render_alpha_overlay_tab(db), budget_ms=400)
