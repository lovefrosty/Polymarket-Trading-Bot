...
class TestWSStarvationDetectorV1(unittest.TestCase):
    def _build_runtime(self, db_path: Path) -> RuntimeEngine:
        ...
        return RuntimeEngine(
            ...
            constitution={
                ...
                "policy": {},
                "trading": {"clock_drift_max_ms": 250.0, "ws_starvation_max_ms": 100.0},
            },
    ...
    def test_starvation_triggers_freeze(self) -> None:
            ...
            asyncio.run(
                runtime.run_stats_cycle(
                    ...
                    liveness_inputs={
                        ...
                        "active_market_lag_ms": 1.0,
                    },
                )
            )
            row = runtime.db.query(
                "SELECT freeze_state, ws_starvation_token_count, reason_codes FROM liveness_stats ORDER BY ts_ms DESC LIMIT 1"
            )[0]
            self.assertEqual(int(row[0]), 1)
            self.assertEqual(int(row[1]), 1)
            self.assertIn("E_WS_STARVATION", str(row[2]))
