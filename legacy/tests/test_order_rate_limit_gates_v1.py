...
class TestOrderRateLimitGatesV1(unittest.TestCase):
    ...
    def test_order_rate_limit_triggers_risk_reason(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            runtime = self._build_runtime(Path(tmp) / "runtime.db")
            now_ms = int(time.time() * 1000)
            runtime._submit_event_ts.append(now_ms)
            reasons = runtime._risk_budget_reasons(now_ms)
            self.assertIn("RISK_ORDER_RATE_LIMIT", reasons)
