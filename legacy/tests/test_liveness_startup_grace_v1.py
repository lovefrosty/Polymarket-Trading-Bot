...
class TestLivenessStartupGraceV1(unittest.TestCase):
    def test_startup_grace_suppresses_liveness_freeze(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            runtime = RuntimeEngine(
                ...
                constitution={
                    ...
                    "trading": {
                        ...
                        "ws_starvation_max_ms": 10_000.0,
                        "startup_liveness_grace_ms": 60_000,
                    },
                },
            ...
            row = runtime.db.query(
                "SELECT freeze_state, reason_codes, payload_json FROM liveness_stats ORDER BY ts_ms DESC LIMIT 1"
            )[0]
            self.assertEqual(int(row[0]), 0)
            self.assertIn("E_CLOCK_DRIFT_HIGH", str(row[1]))
            alerts = runtime.db.query(
                "SELECT COUNT(*) FROM alerts WHERE code='LIVENESS_DEGRADED_STARTUP'"
            )
