...
- Maker-checker is mandatory for any research or trading decision loop. The
  agent that generates a signal may not approve or execute that signal.
- Losses become rules: when a loop fails, the failure must be recorded as a new
  durable operating rule or test case before the loop is trusted again.
- Stopping conditions, promotion gates, and kill-switches must be externally
  checkable. An agent's own claim that it is done is never sufficient.
- These rules apply before new strategy ideas, new data sources, or new
  automation layers are added.
