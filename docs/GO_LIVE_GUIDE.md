# Go-Live Reference Guide: Kalshi First, Polymarket CLOB V2 Later

> **Status as of August 6, 2026:** Kalshi is the current research and paper-trading lane. The Polymarket material later in this document describes the retired CLOB V1 integration and must not be used for production. The repository still pins `py-clob-client==0.20.0`; production Polymarket now requires CLOB V2, pUSD collateral handling, dynamic per-market fees, current signing/order fields, and fresh end-to-end validation.
>
> Keep live execution disabled until the selected venue has repeatable after-fee paper evidence and a separately reviewed low-notional canary plan. For the portfolio-level evidence and limitations, start with the repository `README.md`.
...
secondary-venue reference. It is archived CLOB V1 material, not a current
operating procedure.
