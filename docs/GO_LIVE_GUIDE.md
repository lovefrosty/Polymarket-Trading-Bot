# Archived CLOB V1 Go-Live Guide — Do Not Use for Production

> **Status as of August 6, 2026:** this document describes the pre-April 28, 2026 Polymarket CLOB V1 integration. The repository pins `py-clob-client==0.20.0`; Polymarket production now requires CLOB V2, a different SDK and signing model, pUSD collateral handling, current per-market fees, and fresh end-to-end validation. The historical paper claims below were generated by the local simulator and do not establish live profitability. Keep `LIVE` mode disabled until the migration and a separately reviewed low-notional canary are complete.
>
> This guide is retained as implementation history. For the current boundary and portfolio-level evidence, start with the repository `README.md`.

# Go-Live Reference Guide: Polymarket Market Making Bot (Historical V1)
# Last updated: 2026-03-17
# Status: Pre-deployment

---

## Historical overview (claims not revalidated)

The original guide stated that paper trading was validated using the following simulator outputs:
...
These figures are preserved as historical notes, but the committed run summaries do not support every claim above and none are exchange-confirmed. The remainder of this document is not a current operating procedure.
