...
def test_runner_boundary_no_new_risk_controls_are_runtime_configurable(selector: MarketSelector) -> None:
    runner = CoreMMRunner(
        market_selector=selector,
        boundary_no_new_risk_min_price=0.12,
        boundary_no_new_risk_max_price=0.88,
        boundary_guard_mode="static",
        boundary_adverse_selection_threshold=0.44,
        boundary_exit_cost_multiplier=1.5,
    )

    assert runner.control_state()["boundary_no_new_risk_min_price"] == pytest.approx(0.12)
    assert runner.control_state()["boundary_no_new_risk_max_price"] == pytest.approx(0.88)
    assert runner.control_state()["boundary_guard_mode"] == "static"
    assert runner.control_state()["boundary_adverse_selection_threshold"] == pytest.approx(0.44)
    assert runner.control_state()["boundary_exit_cost_multiplier"] == pytest.approx(1.5)

    applied = runner.apply_config_patch(
        {
            "boundary_no_new_risk_min_price": 0.15,
            "boundary_no_new_risk_max_price": 0.85,
            "boundary_guard_mode": "adaptive",
            "boundary_adverse_selection_threshold": 0.52,
            "boundary_exit_cost_multiplier": 1.75,
        }
    )

    assert applied["boundary_no_new_risk_min_price"] == pytest.approx(0.15)
    assert applied["boundary_no_new_risk_max_price"] == pytest.approx(0.85)
    assert applied["boundary_guard_mode"] == "adaptive"
    assert applied["boundary_adverse_selection_threshold"] == pytest.approx(0.52)
    assert applied["boundary_exit_cost_multiplier"] == pytest.approx(1.75)
    assert runner.control_state()["boundary_no_new_risk_min_price"] == pytest.approx(0.15)
    assert runner.control_state()["boundary_no_new_risk_max_price"] == pytest.approx(0.85)

    rejected = runner.apply_config_patch({"boundary_no_new_risk_min_price": 0.90})
    assert "boundary_no_new_risk_min_price" not in rejected
    assert runner.control_state()["boundary_no_new_risk_min_price"] == pytest.approx(0.15)
