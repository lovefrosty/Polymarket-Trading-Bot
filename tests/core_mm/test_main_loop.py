...
class TestMainLoop(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_boundary_low_price_blocks_new_buy_risk(self) -> None:
        self.books.apply_snapshot("cheap_yes", bids=[(0.09, 150)], asks=[(0.11, 160)], ts_ms=1_000)
        market = MarketConfig(
            market_id="m_boundary_low",
            token_ids=("cheap_yes",),
            tick_size=0.01,
            min_size=100,
            fallback_size=20,
        ...
        result = await loop.run_market_cycle(
            market=market,
            token_states=(TokenState(token_id="cheap_yes", position=0, avg_cost=0.0, net_position=0, usdc_balance=1000),),
            existing_orders={},
            now_ms=2_000,
        )
        self.assertFalse(any(q.side == "buy" for q in result.desired_quotes.values()))

    async def test_boundary_low_price_still_allows_inventory_exit(self) -> None:
        self.books.apply_snapshot("cheap_exit", bids=[(0.09, 150)], asks=[(0.11, 160)], ts_ms=1_000)
        market = MarketConfig(
            market_id="m_boundary_exit",
            token_ids=("cheap_exit",),
            tick_size=0.01,
            min_size=100,
            fallback_size=20,
            within_pct=0.05,
        ...
        result = await loop.run_market_cycle(
            market=market,
            token_states=(TokenState(token_id="cheap_exit", position=20, avg_cost=0.08, net_position=20, usdc_balance=1000),),
            existing_orders={},
            now_ms=2_000,
        )
        self.assertFalse(any(q.side == "buy" for q in result.desired_quotes.values()))
        sell_quote = next(q for q in result.desired_quotes.values() if q.side == "sell")
        self.assertTrue(sell_quote.metadata.get("price_boundary_active"))
        self.assertTrue(sell_quote.metadata.get("price_boundary_buy_blocked"))
        self.assertEqual(sell_quote.metadata.get("price_boundary_reason"), "adaptive_tail_adverse_selection_low")

    async def test_boundary_low_price_allows_risk_reducing_buy(self) -> None:
        self.books.apply_snapshot("cheap_reduce", bids=[(0.09, 150)], asks=[(0.11, 160)], ts_ms=1_000)
        market = MarketConfig(
            market_id="m_boundary_reduce",
            token_ids=("cheap_reduce",),
            tick_size=0.01,
            min_size=100,
            fallback_size=20,
            within_pct=0.05,
        ...
        buy_quote = next(q for q in result.desired_quotes.values() if q.side == "buy")
        self.assertTrue(buy_quote.metadata.get("price_boundary_active"))
        self.assertFalse(buy_quote.metadata.get("price_boundary_buy_blocked"))

    async def test_boundary_high_price_blocks_new_buy_risk(self) -> None:
        self.books.apply_snapshot("rich_yes", bids=[(0.89, 150)], asks=[(0.91, 160)], ts_ms=1_000)
        market = MarketConfig(
            market_id="m_boundary_high",
            token_ids=("rich_yes",),
            tick_size=0.01,
            min_size=100,
            fallback_size=20,
        ...
        result = await loop.run_market_cycle(
            market=market,
            token_states=(TokenState(token_id="rich_yes", position=0, avg_cost=0.0, net_position=0, usdc_balance=1000),),
            existing_orders={},
            now_ms=2_000,
        )
        self.assertFalse(any(q.side == "buy" for q in result.desired_quotes.values()))
