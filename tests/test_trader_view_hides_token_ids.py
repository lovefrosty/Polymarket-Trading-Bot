...
class TestTraderViewHidesIds(unittest.TestCase):
    def test_trader_view_omits_token_and_decision_ids(self) -> None:
        df = pd.DataFrame(
            [
                {
        ...
        out = build_signals_table_for_view(df, "trader", registry)
