        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                discovery_rows = cx.execute(
                    "SELECT reason_code, counts_json FROM discovery_requests ORDER BY ts_ms DESC LIMIT 1"
                ).fetchall()
                self.assertEqual(len(discovery_rows), 1)
                self.assertEqual(discovery_rows[0][0], "NO_ACTIVE_BTC_15M")
                counts = json.loads(discovery_rows[0][1])
